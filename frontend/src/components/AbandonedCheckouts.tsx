...
export const AbandonedCheckouts = ({
  ...
  // Track which specific row is loading
  const [sendingId, setSendingId] = useState<string | number | null>(null);
  // Track which rows have already been "sent"
  const [sentIds, setSentIds] = useState<Set<string | number>>(new Set());

  const handleClick = async (id: string | number, email: string | null) => {
    if (!email) return;

    setSendingId(id); // Start spinning
    await onRecover(id, email); // Wait for parent to finish API call
    setSendingId(null); // Stop spinning

    setSentIds((prev) => new Set(prev).add(id)); // Mark as sent (Green checkmark)
  };
